...
* **Session Freshness & Stale Level Suppression:** Queries strictly for active trading session date (`datetime::date = :target_date`) computed via Eastern Time market calendar. If levels for today's session have not yet been published, returns empty levels rather than falling back to historical dates, and suppresses proximity evaluation when `session_date < expected_date`.
* **Endpoints:**
